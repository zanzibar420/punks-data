...
// Retry with exponential backoff wrapper WITH TIMEOUT
async function retryWithBackoff(fn, retries = 3, delay = 1000, maxTotalTime = 30000) {
  ...
  for (let i = 0; i < retries; i++) {
    try { 
      ...
      if (elapsedTime >= maxTotalTime) {
        console.error(chalk.red(`  ⏱️ Max retry time (${maxTotalTime}ms) exceeded`));
        throw new Error(`Timeout after ${maxTotalTime}ms: ${e.message}`);
      }
      
      if (i === retries - 1) throw e;
      
      const backoffDelay = Math.min(delay * (i + 1), 10000); // Cap at 10 seconds
      console.log(chalk.yellow(`  ⏳ Backing off for ${backoffDelay}ms... (elapsed: ${elapsedTime}ms)`));
...
let lastSuccessfulMintTime = Date.now();
...
async function mintSingleToken(wallet, contract, tokenData, retryCount = 0) {
  ...
  try {
    ...
    // Use retry wrapper with timeout for the actual mint call
    const tx = await retryWithBackoff(async () => {
      const nonce = await wallet.getNonce();
      return contract.mintURI(wallet.address, metadataUri, { nonce });
    }, 3, 1000, 30000); // Max 30 seconds total retry time
    ...
    lastSuccessfulMintTime = Date.now();
...
async function mintPendingTokens(wallet, contract) {
  ...
  for (let i = 0; i < pendingTokens.length; i += CONFIG.BATCH_SIZE) {
    ...
    const timeSinceLastSuccess = Date.now() - lastSuccessfulMintTime;
    if (timeSinceLastSuccess > 300000) { // 5 minutes
      console.error(chalk.red(`\n⚠️ No successful mints in ${Math.floor(timeSinceLastSuccess/60000)} minutes. Exiting to allow restart.`));
      process.exit(1);
    }

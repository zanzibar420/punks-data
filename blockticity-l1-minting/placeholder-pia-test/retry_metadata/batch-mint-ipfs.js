...
const CONFIG = {
  ...
  MINT_LOG_PATH: path.join(__dirname, 'mint_log_ipfs.json'),
  CONTRACT_ABI: [
    'function mintURI(address to, string uri) public',
    'function owner() public view returns (address)',
    'function paused() public view returns (bool)'
  ],
  // Base IPFS gateway for metadata
...
async function initializeBlockchain() {
  ...
  try {
    ...
    console.log(chalk.green(`📄 Contract: ${CONFIG.CONTRACT_ADDRESS}`));
  } catch (error) {
...
async function uploadMetadataToPinata(tokenId, metadata) {
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(`Pinata API error: ${response.status} - ${result.error || 'Unknown error'}`);
    }
...
async function mintSingleToken(wallet, contract, tokenData, retryCount = 0) {
  ...
  try {
    console.log(chalk.yellow(`  🎨 Processing token ${tokenId}...`));
    ...
    try {
      ...
      console.log(chalk.yellow(`  ⚠️  Gas estimation failed, using default: 500000`));
      gasLimit = 500000n;
    }
    
    const tx = await contract.mintURI(wallet.address, metadataUri, { gasLimit });
    console.log(chalk.blue(`  ⏳ Tx submitted: ${tx.hash}`));
    ...
    console.log(chalk.green(`  ✅ Token ${tokenId} minted! Block: ${receipt.blockNumber}`));
...
async function mintPendingTokens(wallet, contract, metadataMap) {
  ...
  console.log(chalk.yellow(`\n⚠️  About to mint ${pendingTokens.length} tokens. This will cost gas.`));
...
async function main() {
  ...
  try {
    ...
    if (!process.env.MAINNET_PRIVATE_KEY) {
      throw new Error('MAINNET_PRIVATE_KEY not found in environment variables');
    }
